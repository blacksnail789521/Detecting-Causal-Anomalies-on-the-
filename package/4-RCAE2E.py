...
class RCAE2E:
    ...
    def fit(self):
        ...
        #for r in [117, 119]:
        for r in range((self.r_w) - 1, len(data["time_series"])):
        #for r in range((self.r_w) - 1, self.r_w  - 1 + 4): # execute 4 times
            
            self.TICC_GTC_mode = 0
            self.r = r
            print("#####  r: " + str(self.r))
            
            ### TICC_GTC for multiple runs (r - r_w + 1 ~ r)
            
            new_profile = self.TICC_GTC(data = data["time_series"][(self.r - self.r_w + 1) : (self.r) + 1])
            
            
            ### initialization
            
            if old_profile == 0:
                old_profile = deepcopy(new_profile)
                continue
            
            
            ### compare two recent Profiles
            
            significant_difference = self.compare_two_profiles(old_profile, new_profile)
            
            
            ### check whether there is a significant difference
            
            if significant_difference == False:
                ...
                print("find anomalous run data! (r = " + str(self.r) + ")")
                
                self.TICC_GTC_mode = 1
                
                
                ### anomalous run data MRF (r)
                
                print("calculate anomalous run data MRF (r = " + str(self.r) + ")")
                
                anomalous_run_data_profile = self.TICC_GTC(data = data["time_series"][(self.r) : (self.r) + 1])
                anomalous_run_data_MRF = []
                for t in range(self.T - self.t_w + 1):
                    cluster_assignment = anomalous_run_data_profile["cluster_assignment"][t]
                    anomalous_run_data_MRF.append( anomalous_run_data_profile["cluster_MRFs"][cluster_assignment] )
                anomalous_run_data_MRF = np.asarray(anomalous_run_data_MRF)
                
                
                ### ground truth run data MRF (old_profile)
                
                ground_truth_run_data_profile = old_profile
                ground_truth_run_data_MRF = []
                for t in range(self.T - self.t_w + 1):
                    ground_truth_run_data_MRF.append([])
                    for ground_truth_r in range( (self.r - self.r_w + 1) + 1 ):
                        cluster_assignment = ground_truth_run_data_profile["cluster_assignment"] \
                                                                          [ground_truth_r * (self.T - self.t_w + 1) + t]
                        ground_truth_run_data_MRF[t].append( ground_truth_run_data_profile["cluster_MRFs"][cluster_assignment] )
                average_ground_truth_run_data_MRF = []
                for element in ground_truth_run_data_MRF:
                    average_ground_truth_run_data_MRF.append( np.sum(element, axis = 0) / np.asarray(element).ndim )
                ground_truth_run_data_MRF = np.asarray(average_ground_truth_run_data_MRF)
                
                
                ### save as pickle
                
                with open(os.path.join(data_folder_path, "test", "data" + ".pickle"), 'wb') as file:
                    pickle.dump(data, file)
                
                with open(os.path.join(data_folder_path, "test", "anomalous_run_data_MRF" + ".pickle"), 'wb') as file:
                    pickle.dump(anomalous_run_data_MRF, file)
                
                with open(os.path.join(data_folder_path, "test", "ground_truth_run_data_MRF" + ".pickle"), 'wb') as file:
                    pickle.dump(ground_truth_run_data_MRF, file)
            ...
            ### copy the value in new_profile to old_profile
