...
class TICC_GTC:
    def __init__(self, \
        ...
        self.TICC_GTC_mode = TICC_GTC_mode
        self.r = r
        self.t = t
        self.T = T
    ...
    def fit(self, data):
        ...
        ### if TICC_GTC_mode = 1, set corresponding parameters to correct config
        
        if self.TICC_GTC_mode == 1:
            
            self.r_w = 1
            #self.K = int(self.T / 2 / 2 / 2)
            self.K = 8
            self.beta = 0
            self.alpha = 0
        ...
        while self.execution_complete != True:
        
            try:
                
                profile = self.calculate_TICC_GTC(data, output_path_for_one_para_comb)
            
            except np.linalg.LinAlgError as err:
            
                self.K = int(self.K / 2)
                print("##########################")
                print("change K to " + str(self.K))
                print("##########################")
    ...
    def calculate_TICC_GTC(self, \
                           data, \
                           output_path_for_one_para_comb):
    ...
    def prepare_out_directory(self):
        if self.TICC_GTC_mode == 0:
            output_path_for_one_para_comb = os.path.join(self.output_path, \
                                                         ...
                                                         \
                                                         "r=" + str(self.r) + \
                                                         " (run " + str(self.r - self.r_w + 1) + " to run " + str(self.r) + ")")
        elif self.TICC_GTC_mode == 1:
            output_path_for_one_para_comb = os.path.join(self.output_path, \
                                                         "lambda=" + str(self._lambda) + \
                                                         " K=" + str(self.K) + \
                                                         " beta=" + str(self.beta) + \
    ...
    def log_parameters(self):
        ...
        print("T = ", self.T)
        '''
        if self.TICC_GTC_mode == 1 or self.TICC_GTC_mode == 2:
            print("t = ", self.t)
        '''
    ...
    def updateClusters_with_GTC(self, ll_concatenate):
        ...
        def AssignPointsToClusters(ll):
            ...
            for t in range(self.T - self.t_w + 1):
                i = t + 1
                # print percentage
                if i % int((self.T - self.t_w + 1)/20) == 0:
                    percentage = int(i / int((self.T - self.t_w + 1) / 20))
                    sys.stdout.write("\r")
...
if __name__ == "__main__":
    ...
    elif TICC_GTC_mode == 1:
        profile = TICC_GTC_instance.fit(data = data["time_series"][(r) : (r) + 1])